...
if(!file.exists(benchmarkfile))
 { res <- apply(beta_values, 1, function(beta_value){
              ...
              res <- lapply(nCellsVec, function(nCells){
                ...
                H5write.blocks(mat, h5file, nGenes, nCells)
                ...
                timing <- microbenchmark(dd <- h5read.chunked(h5file, "data", list(ridx, NULL), nGenes)
                                         ...
                                         , sub4 <- h5read.chunked(h5file, "data", list(ridx, cidx), nGenes)
                                         , times = ntime, unit = "ms")
                ...
                if(!all.equal(sub1, sub2, check.attributes = F)||!all.equal(sub1, sub4, check.attributes = F))
                  stop("wrong reading!")
                thisRes <- rbindlist(list(dt1, dt3, dt5))
